...
Every row below was produced by driving the **real Flutter web build against a real
...
- **9 defects were found and fixed in this pass**, each with a regression test — see
...
| Release APK under test | `app/build/app/outputs/flutter-apk/app-release.apk`, unchanged by this QA pass |
...
The CDP driver that clicks through the app is not part of the repository: it is about 250 lines
of Node with no npm install, and every screenshot it took can be reproduced by hand from the
...
|---|---|---|---|---|---|
| 1a | first launch | "language" | Five languages offered: English, हिन्दी, मराठी, தமிழ், বাংলা. Hard redirect to onboarding until it completes (`core/router.dart`). | **PASS** | `assets/screenshots/01-onboarding-language.png` |
| 1b | persona picker | "pick **Parent + Commuter**" | All 8 personas offered, 1–3 selectable, first pick becomes primary (weight 1.0, others 0.7 per docs/03). | **PASS** | `assets/screenshots/02-onboarding-personas.png` |
| 1c | location | "location Delhi (GPS or search)" | GPS, free-text search and a popular-cities list all present; New Delhi resolved to 28.61/77.21, Asia/Kolkata. | **PASS** | `assets/screenshots/03-onboarding-location.png` |
| 1d | first home | — | Feed renders against the live backend; green dot on the freshness chip = `/ws/alerts` connected. | **PASS** | `assets/screenshots/04-onboarding-first-home.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 2a | parent+commuter, demo clock 07:30 | "School commute + Commute conditions on top" | Ranks 1 and 2 exactly: `school_commute` **0.95**, `commute_conditions` **0.532**. `context.daypart = dawn`. | **PASS** | `assets/screenshots/14-time-override-morning.png` |
| 2b | same | "hero shows now" | Hero reads 27 °C / feels like 31 °C — a **dawn** reading, not the live afternoon observation. This only works because of the QA fix that reads `Snapshot.current` off the forecast hour matching `now_override`. | **PASS** | same |
| 2c | same, scrolled | "with reasons"; "nowcast" | Every card carries its reason chips ("Because you follow Parenting", "The school run needs care (an advisory)"); nowcast present. | **PASS** | `assets/screenshots/15-time-override-morning-scrolled.png` |
| 2d | demo clock 22:00 | docs/03 §Tests 5: school_commute "at 22:00 it is not in the top 3" | Live API, parent + `clear_pleasant`: 07:30 top-3 = `school_commute, health_advisory, nowcast`; 22:00 top-3 = `health_advisory, hourly_forecast, daily_forecast`. Spec test holds against the **running service**, not just the offline fixtures. | **PASS** | `assets/screenshots/16-time-override-night.png` |
| 2e | demo clock, any hour | — | Presets **and** the hand-picked time are stamped with today's date. Was broken; see defect **D5**. | **PASS** (after fix) | `assets/screenshots/40-demo-controls.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 3a | Fitness | "Best workout window, Sun times, Wind, Heat alert move up" | All four rise into the main feed; `heat_alert` pins under the `heatwave` scenario (urgency ≥ 0.8). | **PASS** | `assets/screenshots/08-home-fitness.png` |
| 3b | Health | README step 3: "AQI, pollen, UV, humidity" | All four in the top of the feed. | **PASS** | `assets/screenshots/07-home-health.png` |
| 3c–3h | Parent · Commuter · Traveller · Beach · Agriculture · Event planner | docs/02 §Coverage check | Every persona's coverage cards are reachable; ungated ones reach the top 8 (docs/03 §Tests 7). | **PASS** | `assets/screenshots/0{5,6,9}-home-*.png`, `10-home-beach.png`, `11-home-agriculture.png`, `12-home-event-planner.png` |
| 3i | "View as persona" (demo sheet) | — | Role view swaps the whole feed without touching the saved profile — the request-scoped `?personas=` override. | **PASS** | `assets/screenshots/13-home-view-as-fitness.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 4a | location search | "Change location to Goa" | Search finds Panaji, Goa; switch is instant. | **PASS** | `assets/screenshots/17-coastal-location-search.png` |
| 4b | Panaji home | "Sea conditions, Tides (estimated), Water temp appear" | All three appear; docs/03 §Tests 3 (coastal gating) holds — the same three are absent for Delhi. | **PASS** | `assets/screenshots/18-coastal-home.png` |
| 4c | sea card | — | Wave height / period / direction, swell, sea-surface temperature, sea-state band. Under `cyclone`, `sea_conditions` pins at score 1.0 with a red banner. | **PASS** | `assets/screenshots/19-coastal-sea-conditions.png` |
| 4d | tides card | "(estimated)"; docs/00 principle 6: modelled values are labelled "Estimated" | **One** "Estimated" chip (was two before defect **D2**) plus the backend's own disclaimer: "Estimated from a simplified harmonic model — not an official tide table. Check INCOIS or the Survey of India before…". Nothing modelled is presented as an observation. | **PASS** | `assets/screenshots/20-coastal-tides-estimated.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 5a | before the push | — | No banner, `pinned: []`, `warning_count: 0`. | **PASS** | `assets/screenshots/21-live-warning-before.png` |
| 5b | `POST /admin/warnings`, orange thunderstorm, Delhi | "the phone shows the banner" | Orange banner "Thunderstorm warning — Delhi" arrives from the socket frame *before* `/home` comes back. | **PASS** | `assets/screenshots/22-live-warning-rerank.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 6a | long-press Pollen | ""Why this?"" | Sheet opens with the card's reasons, its score, and what the ranker has learned (taps / dismissals) for that type. | **PASS** | `assets/screenshots/23-why-sheet-pollen.png` |
| 6b | long-press Humidity | — | Same sheet for a second card type. | **PASS** | `assets/screenshots/24-why-sheet-humidity.png` |
| 6c | "Show less" ×n → demote | "it drops to "More for you"" | Measured on live Delhi data, health persona, `clear_pleasant`: `pollen` rank 3 / score **0.470** → after 1 dismiss rank 4 / **0.3804** → 2 dismisses rank 4 / **0.3112** → **3 dismisses rank 8 / 0.2677, now in `more_cards`** → 4 dismisses **0.2437**. Two taps move it but do not always cross; three do. The doc says "twice"; the README already says "two taps move it, three usually push it over" and explains why (docs/03 penalty is `0.25·tanh(x/8)`, so two taps are −0.16 and how far the card must fall depends on its neighbours). | **PASS**, doc wording corrected | `assets/screenshots/25-humidity-demoted.png` |
...
| 6f | pin | README step 6: "the card jumps to the top on the next refresh" | Pin moves the card into the pinned block on the next `/home`; unpin restores. | **PASS** | `assets/screenshots/26-pin-to-top.png` |
| 6g | hide / unhide | docs/04 `hidden_types` | Hidden card leaves the feed and its type comes back in `hidden_types`; restore works. | **PASS** | `assets/screenshots/27-hide-and-restore.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 7a | backend killed, cache warm | "home still renders from cache with the freshness chip" | Full feed renders. Freshness chip reads "**Updated just now · cached**" with a cache icon; the WS dot is gone. One honest strip: "Could not refresh. Showing saved data. · **Retry**". Cache key observed: `flutter.cache_home_28.61_77.21_parent-commuter_en`. | **PASS** | `assets/screenshots/28-offline-cached.png` |
| 7b | backend killed **and** cache cleared | docs/06 §Offline — bundled fixture fallback | Falls through to the bundled `app/assets/fixtures/home_sample.json`. Chip reads "Updated 34 h ago · **Sample data**" and the strip names the unreachable host: "Sample data — the backend at http://127.0.0.1:8000 is not reachable." Nothing pretends to be live. | **PASS** | `assets/screenshots/29-offline-sample-data.png` |
| 7c | banner priority | "bundled sample → offline → stale cache" | One `_FeedStatus` strip, never two, and the wording always matches what is on screen. | **PASS** | both |

The "34 h ago" in 7b is correct and deliberate: a bundled payload has no usable clock of its own, so
it ages against the device clock (deviation recorded in `docs/DEVIATIONS.md`). A *live* payload ages against
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 8a | `lang=hi` home | "chrome … localized" | App chrome fully Hindi: "डेमो नियंत्रण", "सेटिंग्स", "रडार / स्थान / डेमो", "लाइव अपडेट जुड़े हैं", "कार्ड विकल्प", "पिन करें", "हटाएँ", "यह कार्ड छिपाएँ". | **PASS** | `assets/screenshots/30-hindi-home.png` |
| 8b | `lang=hi` cards | "insights localized" | Card titles, subtitles, headlines, details, advice bullets and reason chips all Hindi — e.g. "स्कूल आना-जाना · 07:00–09:00 · सावधानी", "हर 30 मिनट में पानी पिएँ, प्यास न लगे तब भी।", "क्योंकि आप अभिभावक देखते हैं". Measured: **91 of 96** card strings contain Devanagari; **zero** raw i18n keys leak. | **PASS** | `assets/screenshots/31-hindi-cards.png` |
| 8c | `lang=hi` banner | — | Scenario warning copy localized: "नारंगी चेतावनी: गरज के साथ बारिश और बिजली". | **PASS** | — |
| 8d | alert card bands | — | Was **FAIL** — the heat card drew "Extreme Caution" under a subtitle already reading "अत्यधिक सावधानी", and the fog card read "कोहरा warning". Fixed as defect **D4**; re-shot after the fix and the body now reads "अत्यधिक सावधानी". | **PASS** (after fix) | `assets/screenshots/31-hindi-cards.png` |
...
| # | Variant | Expected (quoted) | Observed | Verdict | Screenshot |
|---|---|---|---|---|---|
| 9a | Places page | "saved places Mumbai + London" | Both added through the app's own search (kind `travel`); the page enforces the docs/04 max of 8. Country resolved from the geocode result, so London is "England, United Kingdom". | **PASS** | `assets/screenshots/32-saved-places.png` |
| 9b | traveller home | — | `saved_places` card shows both with their **local** times — Mumbai 17:15 IST, London 12:45 BST — plus temps and rain chance. `travel_alerts` reads "**1 travel alert** — Mumbai" (singular; was "1 travel alert(s)" before defect **D1**). | **PASS** | `assets/screenshots/33-traveler-home.png` |
| 9c | packing | "packing suggestions ("Carry a raincoat in London")" | "What to pack · 5 items", grouped per place: Mumbai → Raincoat / umbrella ("Rain chance up to 100% in 3 days"), Light cotton clothes; **London → Raincoat / umbrella** ("Rain chance up to 53% in 3 days"), Warm layers ("Nights down to 12 °C"), Light cotton clothes. Every item carries its reason. | **PASS** | `assets/screenshots/34-packing-suggestions.png` |
...
| Screenshot | Shows |
|---|---|
| `assets/screenshots/35-scenario-heatwave.png` | Red banner, pinned "Weather warnings (Red · Heatwave)" with `scenario` shown as the source, pinned "Heat alert 49° · Danger" with "Warning in force", re-rank SnackBar |
| `assets/screenshots/36-scenario-dense-fog.png` | Orange banner, pinned commute card at "Severe impact", visibility 0.3 km, timeline in true chronological order with a "Tomorrow" prefix (the QA fix) |
...
| L5 | radar tiles on the card | suppressed | Quick-actions row shows a **Lite** badge in place of Demo | **PASS** | `assets/screenshots/37-low-bandwidth-home.png` |
| L6 | radar tiles on the map page | suppressed | Map page states it plainly: "**Low-bandwidth mode: radar tiles are off.**" Base map still draws, so the warning circles remain readable. | **PASS** | `assets/screenshots/38-low-bandwidth-map.png` |

### Remaining screens

| Screen | Checked | Verdict | Screenshot |
|---|---|---|---|
| Places `/places` | search, kind, delete, max 8 | **PASS** | `assets/screenshots/32-saved-places.png` |
| Map `/map` | OSM base, RainViewer frames, play/scrub, warning circles + markers, user marker | **PASS** | `assets/screenshots/58-early-map-radar.png` (the map page is untouched by this pass) |
| Settings | 5 languages, metric/imperial, 8 personas, home location, school + commute windows, backend URL, low bandwidth, larger text, reset learning, about | **PASS** | `assets/screenshots/39-settings.png` |
| Demo sheet | 10 scenario chips (one per shipped scenario file), 4 clock presets + picker, persona view, simulate offline, low bandwidth, live-alert dot, admin-console link, reset | **PASS** (after **D3**, **D5**, **D6**) | `assets/screenshots/40-demo-controls.png` |
...
| Check | Result |
|---|---|
| Package id | `com.teammausam.mausam_app` — matches docs/00 principle 8 (not an IMD product) |
...
| minSdk / targetSdk / compileSdk | 24 / 36 / 37 — matches the deviations recorded in `docs/DEVIATIONS.md` |
...
| Signature | `CN=Android Debug` — **debug-signed**, as `docs/DEVIATIONS.md` records. A Play-store build needs a real keystore. |
...
regression test, and each is recorded in `docs/DEVIATIONS.md`.
...
Three documentation claims were corrected in the same pass because the walk proved them wrong:
...
| README screenshot grid pointed at `59-early-demo-controls.png` | That shot still showed the Cold wave chip **D3** removed. | `091579e` |
...
2. **`mr`, `ta`, `bn` are best-effort**, 69 app keys and 54 backend keys each, with per-key fallback
   ...
   the design asks for (best-effort locales), not an error. Only **en** and **hi** are complete.
...
5. **Tides, pollen and traffic are modelled**, never observed. Each carries `"source": "estimated"`,
   ...
   and the Survey of India (docs/00 principle 6). Open-Meteo's CAMS pollen fields are `null` for every
...
6. **Admin-pushed and IMD warnings are not localized.** They are free text typed by a human or
   ...
   resolves through the catalog (recorded in `docs/DEVIATIONS.md`).
...
8. **The release APK is debug-signed.** A Play-store build needs a real keystore
   and `key.properties`.
...
| QA screenshots in this pass | **40** (`assets/screenshots/01`–`40`) |
